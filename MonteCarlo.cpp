...
MonteCarlo::MonteCarlo(long long val) : num_trials(val), sim_result(0.0) // put val to num_trials and set pi value(so far) to zero
{
...
void MonteCarlo::runSimulation()
{
    ...
    */
    ...
    for (long long i = 0; i < num_trials; i++) // iterate throw all of the generation trials
    {
...
long long MonteCarlo::returnTrialLim()
{
    return num_trials;
}