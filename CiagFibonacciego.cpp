...
CiagFibonacciego::CiagFibonacciego(int val) : limit(val), fib_values(val + 1, 1) // temporarly create vector with length of val+1 willed with 1(so rar)
{
    fib_values[0] = 0;
...
{
    if (limit > 1) // if leghtn you want to count is bigger than 1
    {
        for (int i = 2; i < limit; ++i) // iterate from 2 up to limit
        {
