...
void SitoEratostenesa::generatePrimary() // find primary numbers in area limited with limit value
{
    ...
    for (int p = 2; p * p <= limit_value; p++) // begin with number p, and then iterate, we are looking for multiply of p, to limit is p*p
    {
        if (primary_web[p] == true)
        {                                                 // if true(so we did not cut from table number yet)
            for (int i = p * p; i <= limit_value; i += p) // we need to cut at the begining square of p number(so i = p*p), then increase i value by next multiply of p
            {
